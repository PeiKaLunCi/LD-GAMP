...
for n_GAMP_layers in range(start_layer, max_n_GAMP_layers + 1, 1):
    ...
    LD_GAMP_D.SetNetworkParams(new_height_img=height_img, new_width_img=width_img, new_channel_img=channel_img, \
                           new_filter_height=filter_height, new_filter_width=filter_width, new_num_filters=num_filters, \
                           new_n_DnCNN_layers=n_DnCNN_layers, new_n_GAMP_layers=n_GAMP_layers,
                           new_sampling_rate=sampling_rate, \
                           new_BATCH_SIZE=BATCH_SIZE, new_sigma_w=sigma_w, new_n=n, new_m=m, new_training=True)
    LD_GAMP_D.ListNetworkParameters()
    ...
    for iter in range(n_layers_trained):
        with tf.variable_scope("Iter" + str(iter)):
            theta_thisIter = LD_GAMP_D.init_vars_DnCNN(init_mu, init_sigma)
        theta[iter] = theta_thisIter

    ## Construct the measurement model and handles/placeholders
    [A_handle, At_handle, A_val, A_val_tf] = LD_GAMP_D.GenerateMeasurementOperators(measurement_mode)

    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! #
    z, z_w, noise_vec, quan_step, DeltaTh, Q_out, y_R, y_measured = LD_GAMP_D.GenerateNoisyCSData_handles(x_true, A_handle, sigma_w, A_val_tf, n_bit)
    #quan_step, y_measured = LD_GAMP_D.GenerateNoisyCSData_handles_Ex(x_true, A_handle, sigma_w, A_val_tf, n_bit)
    ## Construct the reconstruction model
    if alg == 'GAMP':
        (mhat, s_list, mhat_list, vhat_list, V_list, Z_list, ztem_list, vtem_list, t_list, Sigma_list,  R_list, layers_list) = LD_GAMP_D.LDGAMP(y_measured,
            A_handle, At_handle, A_val_tf, theta, x_true, sigma_w, quan_step = quan_step, n_bit = n_bit, tie=tie_weights, training=training_tf,
            LayerbyLayer=LayerbyLayer)

        # (x_hat, MSE_history, NMSE_history, PSNR_history, r_final, rvar_final, div_overN) = LD_GAMP_D.LDGAMP(y_measured,A_handle,At_handle,A_val_tf,theta,x_true, sigma_w, tie=tie_weights,training=training_tf,LayerbyLayer=LayerbyLayer)
    elif alg == 'DIT':
        (x_hat, MSE_history, NMSE_history, PSNR_history) = LD_GAMP_D.LDIT(y_measured, A_handle, At_handle, A_val_tf, theta,
    ...
    nfp = np.float32(height_img * width_img)
    if loss_func == 'SURE':
        assert alg == 'GAMP', "Only LD_GAMP_D supports training with SURE"
        cost = LD_GAMP_D.MCSURE_loss(x_hat, div_overN, r_final, tf.sqrt(rvar_final))
    elif loss_func == 'GSURE':
        assert alg == 'GAMP', "Only LD_GAMP_D currently supports training with GSURE"
        ...
        # Treat LD_GAMP_D/LDIT as a function of A^ty to calculate the divergence
        Aty_tf = At_handle(A_val_tf, y_measured)
        # Overwrite existing x_hat def
        (x_hat, _, _, _, _, _, _) = LD_GAMP_D.LDGAMP_Aty(Aty_tf, A_handle, At_handle, A_val_tf, theta, x_true,
        ...
        if sigma_w == 0.:  # Not sure if TF is smart enough to avoid computing MCdiv when it doesn't have to
            ...
            # Calculate MC divergence of P*LD_GAMP_D(Aty)
            ...
            (x_hat_perturbed, _, _, _, _, _, _) = LD_GAMP_D.LDGAMP_Aty(Aty_perturbed_tf, A_handle, At_handle, A_val_tf,
                                                                  theta, x_true,
        ...
        cost = LD_GAMP_D.MCGSURE_loss(x_hat, x_ML, P, MCdiv, sigma_w)
    ...
    for learning_rate in learning_rates:
        ...
        with tf.Session(config = config) as sess:
            ...
            if ResumeTraining or learning_rate != learning_rates[0]:
                ...
                saver_initvars_name_chckpt = LD_GAMP_D.GenLDGAMPFilename(alg, tie_weights, LayerbyLayer, loss_func=loss_func) + ".ckpt"
                ...
                if InitWeightsMethod == 'layer_by_layer':
                    ...
                    saver_initvars_name_chckpt = LD_GAMP_D.GenLDGAMPFilename(alg, tie_weights, LayerbyLayer=True,
                ...
                if InitWeightsMethod == 'denoiser':
                    ...
                    saver_initvars_name_chckpt = LD_GAMP_D.GenDnCNNFilename(300. / 255.) + ".ckpt"
                    iter = 0
                    for l in range(0, n_DnCNN_layers):
                        saver_dict.update(
                    ...
                    saver_initvars_name_chckpt = LD_GAMP_D.GenLDGAMPFilename(alg, tie_weights, LayerbyLayer,
            ...
            if __name__ == '__main__':
                save_name = LD_GAMP_D.GenLDGAMPFilename(alg, tie_weights, LayerbyLayer, loss_func=loss_func)
                ...
                for offset in range(0, n_Val_Images - BATCH_SIZE + 1,
                    ...
                    A_val = LD_GAMP_D.GenerateMeasurementMatrix(measurement_mode)
                    #A_val = LD_GAMP_D.GenerateMeasurementMatrix_Ex(measurement_mode)
                ...
                if not LayerbyLayer:  # For end-to-end training save the initial state so that LD_GAMP_D end-to-end doesn't diverge when using a high training rate
