...
def SetNetworkParams(new_height_img, new_width_img,new_channel_img, new_filter_height,new_filter_width,\
                     new_num_filters,new_n_DnCNN_layers,new_n_GAMP_layers, new_sampling_rate,\
                     new_BATCH_SIZE,new_sigma_w,new_n,new_m,new_training,use_adaptive_weights=False):
    global height_img, width_img, channel_img, filter_height, filter_width, num_filters, n_DnCNN_layers, n_GAMP_layers,\
...
#Learned DGAMP
def LDGAMP(y,A_handle,At_handle,A_val,theta,x_true, sigma_w, quan_step, n_bit, tie,training=False,LayerbyLayer=True):
    print('------------------------------- LDGAMP -------------------------------')
    ...
    #print('BATCH_SIZE:', BATCH_SIZE)
    #mhat_one =  tf.sqrt(tf.trace(tf.matmul(y, y, transpose_b = True)) / (BATCH_SIZE * n) - sigma_w * sigma_w)
    ...
    for iter in range(n_GAMP_layers):
        ...
        if is_complex:
            ...
            (mhat, vhat, layers) = DnCNN_outer_wrapper(R_list[iter], Sigma_list[iter], theta, tie, iter, training=training, LayerbyLayer=LayerbyLayer)
        ...
        #print("------------------- LDGAMP-14 -------------------", xhat.shape)
        #print("------------------- LDGAMP-15 -------------------", vhat.shape)
...
#Learned DGAMP
def LDGAMP_2(y,A_handle,At_handle,A_val,theta,x_true, sigma_w, quan_step, n_bit, tie,training=False,LayerbyLayer=True):
    print('------------------------------- LDGAMP_2 -------------------------------')
    ...
    for iter in range(n_GAMP_layers):
        ...
        for i in range(5):
            ...
            (mhat, vhat, layers) = DnCNN_outer_wrapper(R, Sigma, theta, tie, iter,
        ...
        (mhat, vhat, layers) = DnCNN_outer_wrapper(R_list[iter], Sigma_list[iter], theta, tie, iter, training=training,
...
def LDGAMP_1(y,theta,x_true, sigma_w, quan_step, n_bit, tie,training=False,LayerbyLayer=True):
    print('------------------------------- LDGAMP_1 -------------------------------')
    s = tf.zeros(shape = (m, BATCH_SIZE), dtype = tf.float32)
    #s = y
    print('s:', s)
...
#Learned DGAMP operating on Aty. Used for calculating MCSURE loss
def LDGAMP_Aty(Aty,A_handle,At_handle,A_val,theta,x_true,tie,training=False,LayerbyLayer=True):
    Atz=Aty
...
#Learned DGAMP
def LDGAMPxsxasxsa(y,A_handle,At_handle,A_val,theta,x_true,tie,training=False,LayerbyLayer=True):
...
def init_vars_mean(n):
    means = None
    with tf.variable_scope('Mean'):
        means = tf.Variable(name = 'mean', initial_value = tf.zeros(shape = (n, 1)), trainable = False)
    return means
...
def GenLDGAMPFilename(alg,tie_weights,LayerbyLayer,n_GAMP_layer_override=None,sampling_rate_override=None,loss_func='MSE'):
    ...
    if loss_func=='SURE':
        filename = "./saved_models/LD_GAMP_D/SURE_"+alg+"_" + str(n_DnCNN_layers) + "DnCNNL_" + str(int(n_GAMP_layers_save)) + "GAMPL_Tie"+str(tie_weights)+"_LbyL"+str(LayerbyLayer)+"_SR" +str(int(sampling_rate_save*100))
    elif loss_func=='GSURE':
        filename = "./saved_models/LD_GAMP_D/GSURE_"+alg+"_" + str(n_DnCNN_layers) + "DnCNNL_" + str(int(n_GAMP_layers_save)) + "GAMPL_Tie"+str(tie_weights)+"_LbyL"+str(LayerbyLayer)+"_SR" +str(int(sampling_rate_save*100))
    else:
        filename = "./saved_models/LD_GAMP_D/"+alg+"_" + str(n_DnCNN_layers) + "DnCNNL_" + str(int(n_GAMP_layers_save)) + "GAMPL_Tie"+str(tie_weights)+"_LbyL"+str(LayerbyLayer)+"_SR" +str(int(sampling_rate_save*100))
    return filename
