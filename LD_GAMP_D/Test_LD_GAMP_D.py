...
if tie_weights == True:
    ...
    noise_min_stds = [0, 10, 20, 40, 60, 80, 100, 150, 300]#This is currently hardcoded within LearnedDGAMP_functionhelper
    noise_max_stds = [10, 20, 40, 60, 80, 100, 150, 300, 500]  # This is currently hardcoded within LearnedDGAMP_functionhelper
    ...
    for noise_level in range(len(noise_min_stds)):
        with tf.variable_scope("Adaptive_NL"+str(noise_level)):
            theta[noise_level]= LD_GAMP_D.init_vars_DnCNN(init_mu, init_sigma)
else:
...
if alg == 'GAMP':
    ...
    #(x_hat, MSE_history, NMSE_history, PSNR_history, r, rvar, dxdr) = LD_GAMP_D.LDGAMP(y_measured, A_handle, At_handle, A_val_tf, theta, x_true, tie=tie_weights)
elif alg == 'DIT':
    (x_hat, MSE_history, NMSE_history, PSNR_history) = LD_GAMP_D.LDIT(y_measured, A_handle, At_handle, A_val_tf, theta, x_true, tie=tie_weights)
else:
...
#with tf.Session() as sess:
    if tie_weights == 1: # Load weights from pretrained denoiser
        save_name = LD_GAMP_D.GenDnCNNFilename(80. / 255.) + ".ckpt"
        ...
        for noise_level in range(len(noise_min_stds)):
            ...
            save_name = LD_GAMP_D.GenDnCNNFilename(noise_min_std/ 255.,noise_max_std/255.) + ".ckpt"
            for l in range(0, n_DnCNN_layers):
        ...
        print('Restore !!!')
        #save_name = LD_GAMP_D.GenLDGAMPFilename(alg, tie_weights, LayerbyLayer) + ".ckpt"
        save_name = LD_GAMP_D.GenLDGAMPFilename(alg, tie_weights, LayerbyLayer,sampling_rate_override=sampling_rate_train,loss_func=TrainLoss) + ".ckpt"
