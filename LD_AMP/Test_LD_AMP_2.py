...
if tie_weights == True:
    ...
    for noise_level in range(len(noise_min_stds)):
        with tf.variable_scope("Adaptive_NL"+str(noise_level)):
            theta[noise_level]= LD_AMP.init_vars_DnCNN(init_mu, init_sigma)
else:
...
if alg == 'DAMP':
    (x_hat, V_list, Z_list, Sigma_list, R_list, xhat_list, vhat_list, layers_list, dw) = LD_AMP.LDAMP(y_measured, A_handle, At_handle, A_val_tf, theta, x_true, sigma_w, tie=tie_weights)
    #(x_hat, MSE_history, NMSE_history, PSNR_history, r, rvar, dxdr) = LD_AMP.LDAMP(y_measured, A_handle, At_handle, A_val_tf, theta, x_true, tie=tie_weights)
elif alg == 'DIT':
    (x_hat, MSE_history, NMSE_history, PSNR_history) = LD_AMP.LDIT(y_measured, A_handle, At_handle, A_val_tf, theta, x_true, tie=tie_weights)
else:
...
#with tf.Session() as sess:
    if tie_weights == 1: # Load weights from pretrained denoiser
        save_name = LD_AMP.GenDnCNNFilename(80. / 255.) + ".ckpt"
        ...
        for noise_level in range(len(noise_min_stds)):
            ...
            save_name = LD_AMP.GenDnCNNFilename(noise_min_std/ 255.,noise_max_std/255.) + ".ckpt"
            for l in range(0, n_DnCNN_layers):
        ...
        print('Restore !!!')
        #save_name = LD_AMP.GenLDAMPFilename(alg, tie_weights, LayerbyLayer) + ".ckpt"
        save_name = LD_AMP.GenLDAMPFilename(alg, tie_weights, LayerbyLayer,sampling_rate_override=sampling_rate_train,loss_func=TrainLoss) + ".ckpt"
    ...
    for offset in range(0, n_Test_Images - BATCH_SIZE + 1, BATCH_SIZE):  # Subtract batch size-1 to avoid eerrors when len(train_images) is not a multiple of the batch size
        ...
        batch_x_recon = sess.run(x_hat, feed_dict={x_true: batch_x_test, A_val_tf: A_val})
    ...
    MSE = np.mean(np.square((255 * (x1 - x2))))
    psnr = 10 * np.log(255 * 255 / MSE) / np.log(10)
    print(psnr)

    [_, _, PSNR] = LD_AMP.EvalError_np(x1, x2)
    print(PSNR)