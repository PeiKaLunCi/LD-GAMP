...
y_measured = LD_AMP.AddNoise(x_true,sigma_w_tf)
...
#x_hat = LD_AMP.DnCNN(y_measured,None,theta_dncnn,training=training_tf)
[x_hat, div_overN, layers] = LD_AMP.DnCNN_wrapper(y_measured,None,theta_dncnn,training=training_tf)

## Define loss and optimizer

nfp=np.float32(height_img*width_img)
if useSURE:
    cost = LD_AMP.MCSURE_loss(x_hat,div_overN,y_measured,sigma_w_tf)
else:
...
for learning_rate in learning_rates:
    ...
    with tf.Session(config=config) as sess:
        ...
        if ResumeTraining or learning_rate!=learning_rates[0]:
            ...
            saver_initvars_name_chckpt=LD_AMP.GenDnCNNFilename(sigma_w_min,sigma_w_max,useSURE=useSURE)+ ".ckpt"
        ...
        if __name__ == '__main__':
            save_name = LD_AMP.GenDnCNNFilename(sigma_w_min,sigma_w_max,useSURE=useSURE)
