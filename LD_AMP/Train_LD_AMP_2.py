...
from LD_GAMP_R import LD_GAMP_R as LD_GAMP_R
...
max_Epoch_Fails=10#How many training epochs to run without improvement in the validation error
...
#learning_rates = [0.001, 0.00001]#, 0.00001]
learning_rates = [0.0005, 0.000005]#, 0.00001]
...
for n_DAMP_layers in range(start_layer,max_n_DAMP_layers+1,1):
    ...
    #y_measured = LD_AMP.GenerateNoisyCSData_handles(x_true, A_handle, sigma_w, A_val_tf)

    ## Construct the reconstruction model
    if alg=='DAMP':
        (x_hat, V_list, Z_list, Sigma_list, R_list, xhat_list, vhat_list, layers_list, dw) = LD_AMP.LDAMP(y_measured,A_handle,At_handle,A_val_tf,theta,x_true, sigma_w, tie=tie_weights,training=training_tf,LayerbyLayer=LayerbyLayer)
        #(x_hat, MSE_history, NMSE_history, PSNR_history, r_final, rvar_final, div_overN) = LD_AMP.LDAMP(y_measured,A_handle,At_handle,A_val_tf,theta,x_true, sigma_w, tie=tie_weights,training=training_tf,LayerbyLayer=LayerbyLayer)
    elif alg=='DIT':
        (x_hat, MSE_history, NMSE_history, PSNR_history) = LD_AMP.LDIT(y_measured,A_handle,At_handle,A_val_tf,theta,x_true,tie=tie_weights,training=training_tf,LayerbyLayer=LayerbyLayer)
    else:
    ...
    nfp = np.float32(height_img * width_img)
    if loss_func=='SURE':
        assert alg=='DAMP', "Only LD_AMP supports training with SURE"
        cost = LD_AMP.MCSURE_loss(x_hat, div_overN, r_final, tf.sqrt(rvar_final))
    elif loss_func=='GSURE':
        assert alg == 'DAMP', "Only LD_AMP currently supports training with GSURE"
        ...
        #Treat LD_AMP/LDIT as a function of A^ty to calculate the divergence
        Aty_tf=At_handle(A_val_tf,y_measured)
        #Overwrite existing x_hat def
        (x_hat, _, _, _, _, _, _) = LD_AMP.LDAMP_Aty(Aty_tf, A_handle,At_handle,A_val_tf, theta, x_true,tie=tie_weights,training=training_tf,LayerbyLayer=LayerbyLayer)
        if sigma_w==0.:#Not sure if TF is smart enough to avoid computing MCdiv when it doesn't have to
            MCdiv=0.
        else:
            #Calculate MC divergence of P*LD_AMP(Aty)
            ...
            (x_hat_perturbed, _, _, _, _, _, _) = LD_AMP.LDAMP_Aty(Aty_perturbed_tf, A_handle, At_handle, A_val_tf, theta, x_true,
        ...
        cost = LD_AMP.MCGSURE_loss(x_hat,x_ML,P,MCdiv,sigma_w)
    ...
    for learning_rate in learning_rates:
        ...
        #with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            if ResumeTraining or learning_rate!=learning_rates[0]:
                ...
                saver_initvars_name_chckpt = LD_AMP.GenLDAMPFilename(alg, tie_weights, LayerbyLayer,loss_func=loss_func) + ".ckpt"
                ...
                if InitWeightsMethod == 'layer_by_layer':
                    ...
                    saver_initvars_name_chckpt = LD_AMP.GenLDAMPFilename(alg, tie_weights, LayerbyLayer=True,loss_func=loss_func) + ".ckpt"
                    for iter in range(
                ...
                if InitWeightsMethod=='denoiser':
                    ...
                    saver_initvars_name_chckpt=LD_AMP.GenDnCNNFilename(300./255.)+".ckpt"
                    iter = 0
                    for l in range(0, n_DnCNN_layers):
                    ...
                    saver_initvars_name_chckpt = LD_AMP.GenLDAMPFilename(alg, tie_weights, LayerbyLayer,
            ...
            if __name__ == '__main__':
                save_name = LD_AMP.GenLDAMPFilename(alg, tie_weights, LayerbyLayer,loss_func=loss_func)
                ...
                for offset in range(0, n_Val_Images - BATCH_SIZE + 1, BATCH_SIZE):  # Subtract batch size-1 to avoid eerrors when len(train_images) is not a multiple of the batch size
                    ...
                    A_val=LD_AMP.GenerateMeasurementMatrix(measurement_mode)
                    ...
                    #x_hat_val, loss_val, V_list_val, Z_list_val, Sigma_list_val, R_list_val, xhat_list_val, vhat_list_val, layers_list_val = \
                    #    sess.run([x_hat, cost, V_list, Z_list, Sigma_list, R_list, xhat_list, vhat_list, layers_list], feed_dict={x_true: batch_x_val, A_val_tf:A_val, training_tf:False})
                    #print('A_val:')
                    #print(A_val)
                    #A_2 = A_val * A_val
                    #print('A_2:')
                    ...
                    for list_i in range(len(vhat_list_val)):
                        ...
                        #print('......................')
                        #print(np.matmul(A_2, vhat_list_val[list_i]))
                    ...
                    for list_i in range(len(layers_list_val)):
                        ...
                        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
                    """
                    #print('loss_val:', loss_val, 'x_hat_val:', x_hat_val, 'x_true:', batch_x_val)
                ...
                if not LayerbyLayer:#For end-to-end training save the initial state so that LD_AMP end-to-end doesn't diverge when using a high training rate
                ...
                for i in range(EPOCHS):
                    ...
                    for offset in range(0, n_Train_Images-BATCH_SIZE+1, BATCH_SIZE):#Subtract batch size-1 to avoid errors when len(train_images) is not a multiple of the batch size
                        ...
                        _, loss_val = sess.run([optimizer, cost],
                            feed_dict={x_true: batch_x_train, A_val_tf:A_val, training_tf:True})#Feed dict names should match with the placeholders
                        """
                        print('V_list_val:')
                        for list_i in range(len(V_list_val)):
                            print(V_list_val[list_i])
