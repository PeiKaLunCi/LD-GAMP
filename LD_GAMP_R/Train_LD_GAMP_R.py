...
parser.add_argument(
    "--alg",
    type=str,
    default="GAMP",  # Options are GAMP, DIT, DVAMP, and DGAMP
...
parser.add_argument(
    ...
    type=int,
    default=16,
    help="How many ResNet layers to use within each AMP layer")
parser.add_argument(
    "--tie_weights",
    type=bool,
    nargs="?",
...
filter_width = 3
num_filters = 64
#n_ResNet_layers = FLAGS.ResNet_layers
max_n_GAMP_layers = 10
...
if tie_weights == True:
    LayerbyLayer = False
    start_layer = max_n_GAMP_layers
#learning_rates = [0.001, 0.0001]  # , 0.00001]
learning_rates = [0.002, 0.0002]#, 0.00001]
...
for n_GAMP_layers in range(start_layer, max_n_GAMP_layers + 1, 1):
    ...
    if alg == 'GAMP':
        ...
        # (x_hat, MSE_history, NMSE_history, PSNR_history, r_final, rvar_final, div_overN) = LD_GAMP_R.LDGAMP_ResNet(y_measured,A_handle,At_handle,A_val_tf,theta,x_true, sigma_w, tie=tie_weights,training=training_tf,LayerbyLayer=LayerbyLayer)
    elif alg == 'DIT':
        (x_hat, MSE_history, NMSE_history, PSNR_history) = LD_GAMP_R.LDIT(y_measured, A_handle, At_handle, A_val_tf, theta,
                                                                      x_true, tie=tie_weights, training=training_tf,
                                                                      LayerbyLayer=LayerbyLayer)
    else:
    ...
    nfp = np.float32(height_img * width_img)
    if loss_func == 'SURE':
        assert alg == 'GAMP', "Only LDGAMP supports training with SURE"
        cost = LD_GAMP_R.MCSURE_loss(x_hat, div_overN, r_final, tf.sqrt(rvar_final))
    elif loss_func == 'GSURE':
        assert alg == 'GAMP', "Only LDGAMP currently supports training with GSURE"
        ...
        # Treat LD_GAMP_R/LDIT as a function of A^ty to calculate the divergence
        Aty_tf = At_handle(A_val_tf, y_measured)
        # Overwrite existing x_hat def
        (x_hat, _, _, _, _, _, _) = LD_GAMP_R.LDGAMP_ResNet_Aty(Aty_tf, A_handle, At_handle, A_val_tf, theta, x_true,
        ...
        if sigma_w == 0.:  # Not sure if TF is smart enough to avoid computing MCdiv when it doesn't have to
            ...
            # Calculate MC divergence of P*LD_GAMP_R(Aty)
            ...
            (x_hat_perturbed, _, _, _, _, _, _) = LD_GAMP_R.LDGAMP_ResNet_Aty(Aty_perturbed_tf, A_handle, At_handle, A_val_tf,
                                                                  theta, x_true,
        ...
        cost = LD_GAMP_R.MCGSURE_loss(x_hat, x_ML, P, MCdiv, sigma_w)
        # Note: This cost is missing a ||Px||^2 term and so is expected to go negative
    else:
        # print('x_hat:', x_hat)
        cost = tf.nn.l2_loss(x_true - mhat) * 1. / nfp

    iter = n_GAMP_layers - 1
    if LayerbyLayer == True:
        vars_to_train = []  # List of only the variables in the last layer.

        vars_to_train.extend([theta[iter][0][0], theta[iter][1][0]])

        num = 9
        for i in range(1, num):
            for j in range(3):
                vars_to_train.extend([theta[iter][0][3 * (i - 1) + j + 1], theta[iter][1][3 * (i - 1) + j + 1]])

                tmp = "Iter" + str(iter) + "/conv_" + str(i) + "/BN" + str(j)
                gamma_name = tmp + "/gamma:0"
                beta_name = tmp + "/beta:0"
                var_name = tmp + "/moving_variance:0"
                mean_name = tmp + "/moving_mean:0"
        ...
        vars_to_train.extend([theta[iter][0][-1], theta[iter][1][-1]])
    ...
    for learning_rate in learning_rates:
        ...
        with tf.Session(config = config) as sess:
            ...
            if ResumeTraining or learning_rate != learning_rates[0]:
                ...
                saver_initvars_name_chckpt = LD_GAMP_R.GenLDGAMP_ResNetFilename(alg, tie_weights, LayerbyLayer, loss_func=loss_func) + ".ckpt"

                for iter in range(n_layers_trained):  # Create a dictionary with all the variables except those associated with the optimizer.

                    i = 0
                    saver_dict.update({"Iter" + str(iter) + "/conv_" + str(i) + "/w": theta[iter][0][0]})
                    saver_dict.update({"Iter" + str(iter) + "/conv_" + str(i) + "/b": theta[iter][1][0]})

                    num = 9
                    for i in range(1, num):
                        for j in range(3):
                            saver_dict.update({"Iter" + str(iter) + "/conv_" + str(i) + "/w" + str(j): theta[iter][0][3 * (i - 1) + j + 1]})
                            saver_dict.update({"Iter" + str(iter) + "/conv_" + str(i) + "/b" + str(j): theta[iter][1][3 * (i - 1) + j + 1]})
                            ...
                            saver_dict.update({tmp + "/gamma": gamma})
                            saver_dict.update({tmp + "/beta": beta})
                            saver_dict.update({tmp + "/moving_variance": moving_variance})
                            saver_dict.update({tmp + "/moving_mean": moving_mean})

                    i = num
                    saver_dict.update({"Iter" + str(iter) + "/conv_" + str(i) + "/w": theta[iter][0][-1]})
                    saver_dict.update({"Iter" + str(iter) + "/conv_" + str(i) + "/b": theta[iter][1][-1]})
                ...
                # To do so, one associates each variable with a key (e.g. theta[iter][0][0] with l1/w_ResNet) and loads the l1/w_DCNN weights that were trained on the denoiser
                ...
                if InitWeightsMethod == 'layer_by_layer':
                    ...
                    saver_initvars_name_chckpt = LD_GAMP_R.GenLDGAMP_ResNetFilename(alg, tie_weights, LayerbyLayer=True,
                    ...
                    for iter in range(
                        ...
                        for l in range(0, n_ResNet_layers):  # Associate variance, means, and beta
                ...
                if InitWeightsMethod == 'denoiser':
                    ...
                    saver_initvars_name_chckpt = LD_GAMP_R.GenResNetFilename(300. / 255.) + ".ckpt"
                    iter = 0
                    for l in range(0, n_ResNet_layers):
                        saver_dict.update(
                            {"l" + str(l) + "/w": theta[iter][0][l]})  # , "l" + str(l) + "/b": theta[iter][1][l]})
                    ...
                    saver_initvars = tf.train.Saver(saver_dict)
                    saver_initvars.restore(sess, saver_initvars_name_chckpt)
                elif InitWeightsMethod == 'smaller_net' and n_GAMP_layers != 1:
                    # Initialize wieghts using a smaller network's weights
                    saver_initvars_name_chckpt = LD_GAMP_R.GenLDGAMP_ResNetFilename(alg, tie_weights, LayerbyLayer,
                                                                        n_GAMP_layer_override=n_GAMP_layers - 1,
                                                                        loss_func=loss_func) + ".ckpt"

                    # Load the first n-1 iterations weights from a previously learned network
                    for iter in range(n_GAMP_layers - 1):
                        ...
                        for i in range(1, num):
                            for j in range(3):
                                saver_dict.update({"Iter" + str(iter) + "/conv_" + str(i) + "/w" + str(j):
                                                       theta[iter][0][3 * (i - 1) + j + 1]})
                                saver_dict.update({"Iter" + str(iter) + "/conv_" + str(i) + "/b" + str(j):
                                                       theta[iter][1][3 * (i - 1) + j + 1]})
                    ...
                    i = 0
                    saver_dict.update({"Iter" + str(iter - 1) + "/conv_" + str(i) + "/w": theta[iter][0][0]})
                    saver_dict.update({"Iter" + str(iter - 1) + "/conv_" + str(i) + "/b": theta[iter][1][0]})

                    num = 9
                    for i in range(1, num):
                        for j in range(3):
                            saver_dict.update({"Iter" + str(iter - 1) + "/conv_" + str(i) + "/w" + str(j):
                                                   theta[iter][0][3 * (i - 1) + j + 1]})
                            saver_dict.update({"Iter" + str(iter - 1) + "/conv_" + str(i) + "/b" + str(j):
                                                   theta[iter][1][3 * (i - 1) + j + 1]})
                            ...
                            tmp_1 = "Iter" + str(iter - 1) + "/conv_" + str(i) + "/BN" + str(j)

                            saver_dict.update({tmp_1 + "/gamma": gamma})
                            saver_dict.update({tmp_1 + "/beta": beta})
                            saver_dict.update({tmp_1 + "/moving_variance": moving_variance})
                            saver_dict.update({tmp_1 + "/moving_mean": moving_mean})

                    i = num
                    saver_dict.update({"Iter" + str(iter - 1) + "/conv_" + str(i) + "/w": theta[iter][0][-1]})
                    saver_dict.update({"Iter" + str(iter - 1) + "/conv_" + str(i) + "/b": theta[iter][1][-1]})
            ...
            if __name__ == '__main__':
                save_name = LD_GAMP_R.GenLDGAMP_ResNetFilename(alg, tie_weights, LayerbyLayer, loss_func=loss_func)
                ...
                for offset in range(0, n_Val_Images - BATCH_SIZE + 1,
                    ...
                    """
                    loss_val, mhat_val, y_measured_val, s_list_val, mhat_list_val, vhat_list_val, V_list_val, Z_list_val, ztem_list_val, vtem_list_val, t_list_val, Sigma_list_val, R_list_val, layers_list_val = \
                        sess.run([cost, mhat, y_measured, s_list, mhat_list, vhat_list, V_list, Z_list, ztem_list, vtem_list, t_list, Sigma_list,  R_list, layers_list],
                    ...
                    print('mhat_val:', mhat_val)
                    print('y_measured_val:', y_measured_val)


                    print('s_list_val:')
                    for list_i in range(len(s_list_val)):
                        print(s_list_val[list_i])
                    print('mhat_list_val:')
                    for list_i in range(len(mhat_list_val)):
                        print(mhat_list_val[list_i])
                    print('vhat_list_val:')
                    for list_i in range(len(vhat_list_val)):
                        print(vhat_list_val[list_i])
                    print('V_list_val:')
                    for list_i in range(len(V_list_val)):
                        print(V_list_val[list_i])
                    print('Z_list_val:')
                    for list_i in range(len(Z_list_val)):
                        print(Z_list_val[list_i])
                    print('ztem_list_val:')
                    for list_i in range(len(ztem_list_val)):
                        print(ztem_list_val[list_i])
                    print('vtem_list_val:')
                    for list_i in range(len(vtem_list_val)):
                        print(vtem_list_val[list_i])
                    print('t_list_val:')
                    for list_i in range(len(t_list_val)):
                        print(t_list_val[list_i])
                    print('Sigma_list_val:')
                    for list_i in range(len(Sigma_list_val)):
                        print(Sigma_list_val[list_i])
                    print('R_list_val:')
                    for list_i in range(len(R_list_val)):
                        print(R_list_val[list_i])

                    #print('layers_list_val:')
                    #for list_i in range(len(layers_list_val)):
                    #    for list_j in range(len(layers_list_val[list_i])):
                    #        print(layers_list_val[list_i][list_j])
                    #    print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')

                    raise NameError

                    """
                    #_, loss_val = sess.run([x_hat, cost], feed_dict={x_true: batch_x_val, A_val_tf:A_val, training_tf:False})
                    _, loss_val = sess.run([mhat, cost],
                                           feed_dict={x_true: batch_x_val, A_val_tf: A_val, training_tf: False})
                    val_values.append(loss_val)
                time_taken = time.time() - start_time
                print("np.mean(val_values):", np.mean(val_values))
                # xsxasasasasasasasasasasasasasasasasasasasasx
                if not LayerbyLayer:  # For end-to-end training save the initial state so that LD_GAMP_R end-to-end doesn't diverge when using a high training rate
                ...
                for i in range(EPOCHS):
                    ...
                    for offset in range(0, n_Train_Images - BATCH_SIZE + 1,
                        ...
                        # Run optimization. This will both generate compressive measurements and then recontruct from them.
                        # _, x_hat_val, loss_val, V_list_val, Z_list_val, Sigma_list_val, R_list_val, xhat_list_val, vhat_list_val, layers_list_val \
                        #    = sess.run([optimizer, x_hat, cost, V_list, Z_list, Sigma_list, R_list, xhat_list, vhat_list, layers_list],
                        ...
                        for list_i in range(len(R_list_val)):
                            print(R_list_val[list_i])
                        print('xhat_list_val:')
                        for list_i in range(len(xhat_list_val)):
                            print(xhat_list_val[list_i])
                        print('vhat_list_val:')
                        for list_i in range(len(vhat_list_val)):
                            print(vhat_list_val[list_i])
                        print('layers_list_val:')
                        ...
                        print('loss_val2:', loss_val, 'x_hat_val2:', x_hat_val, 'x_true2:', batch_x_train)
                        """
                    ...
                    for offset in range(0, n_Val_Images - BATCH_SIZE + 1,
                        ...
                        # print('loss_val3:', loss_val, 'x_hat_val3:', x_hat_val, 'x_true3:', batch_x_val)
                        val_values.append(loss_val)
                    time_taken = time.time() - start_time
