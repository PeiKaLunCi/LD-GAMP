...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    #sum_mean = 0.0
    #sum_var = 0.0
    ResNet_Reconstructing = np.zeros(shape = (height_img * width_img * channel_img, n_Val_Images), dtype = np.float32)
    for offset in range(0, n_Val_Images - BATCH_SIZE + 1, BATCH_SIZE):
        ...
        ResNet_Reconstructing[:, offset: end] = reconstructed_test_images
    ...
    print('ResNet_Reconstructing.shape:', ResNet_Reconstructing.shape)
    np.save('ResNet_Reconstructing.npy', ResNet_Reconstructing)
    """
    print('offset:', offset)
    print('end:', end)
