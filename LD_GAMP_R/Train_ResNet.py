...
LD_GAMP_R.SetNetworkParams(new_height_img=height_img, new_width_img=width_img, new_channel_img=channel_img, \
                       ...
                       new_n_GAMP_layers=None, new_sampling_rate=None, \
...
y_measured, noise_vec = LD_GAMP_R.AddNoise(x_true,sigma_w_tf)
...
x_hat, layers = LD_GAMP_R.ResNet(y_measured,None,theta_dncnn,training=training_tf)

## Define loss and optimizer

nfp=np.float32(height_img*width_img)
if useSURE:
    cost = LD_GAMP_R.MCSURE_loss(x_hat,div_overN,y_measured,sigma_w_tf)
else:
...
for learning_rate in learning_rates:
    ...
    with tf.Session(config=config) as sess:
        ...
        if ResumeTraining or learning_rate!=learning_rates[0]:
            ...
            #print('theta_dncnn:', theta_dncnn)

            ##Load previous values for the weights and BNs
            saver_initvars_name_chckpt=LD_GAMP_R.GenResNetFilename(sigma_w_min,sigma_w_max,useSURE=useSURE)+ ".ckpt"

            i = 0
            saver_dict.update({"conv_" + str(i) + "/w": theta_dncnn[0][0]})
            saver_dict.update({"conv_" + str(i) + "/b": theta_dncnn[1][0]})

            num = 9
            for i in range(1, num):
                for j in range(3):

                    saver_dict.update({"conv_" + str(i) + "/w" + str(j): theta_dncnn[0][3 * (i - 1) + j + 1]})
                    saver_dict.update({"conv_" + str(i) + "/b" + str(j): theta_dncnn[1][3 * (i - 1) + j + 1]})

                    tmp = "conv_" + str(i) + "/BN" + str(j)
            ...
            i = num
            saver_dict.update({"conv_" + str(i) + "/w": theta_dncnn[0][-1]})
            saver_dict.update({"conv_" + str(i) + "/b": theta_dncnn[1][-1]})

            #print('saver_dict:', saver_dict)
        ...
        if __name__ == '__main__':
            save_name = LD_GAMP_R.GenResNetFilename(sigma_w_min,sigma_w_max,useSURE=useSURE)
